...
export default function Edit({ promoCode, plans }: { promoCode: PromoCode; plans: Plan[] }) {
    const { data, setData, put, processing, errors } = useForm({
        code: promoCode.code,
        type: promoCode.type,
        value: promoCode.value.toString(),
        plan_id: promoCode.plan_id?.toString() || '',
        max_uses: promoCode.max_uses?.toString() || '',
        valid_from: promoCode.valid_from || '',
        valid_until: promoCode.valid_until || '',
        first_payment_only: promoCode.first_payment_only,
        is_active: promoCode.is_active,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('admin.promo-codes.update', promoCode.id));
    };

    return (
        <AdminLayout>
            <Head title={`Редактировать промокод: ${promoCode.code}`} />
            ...
            <div className="space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold">Редактировать промокод</h1>
                        <p className="text-muted-foreground">Код: {promoCode.code}</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit}>
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Измените параметры промокода</CardDescription>
                        </CardHeader>
